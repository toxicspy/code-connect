...
interface ShareAIChatDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  aiName: string;
}

const ShareAIChatDialog = ({ open, onOpenChange, aiName }: ShareAIChatDialogProps) => {
  const { user } = useAuth();
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    const message = `🤖 Check out my AI assistant: "${aiName}" — Create your own in the AI Chats tab!`;
    try {
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-base">
            Share <span className="text-primary">{aiName}</span>
          </DialogTitle>
        </DialogHeader>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search contacts..." className="pl-9 bg-muted border-0" />
        </div>
        <div className="max-h-64 overflow-y-auto -mx-2">
          {filtered.length === 0 ? (
            ...
            filtered.map((conv) => (
              <button key={conv.id} onClick={() => toggle(conv.id)} className="flex w-full items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-muted/50">
                ...
                <span className="flex-1 text-left text-sm font-medium truncate">{conv.otherUser.display_name}</span>
        ...
        <DialogFooter>
          <Button onClick={handleSend} disabled={selected.size === 0 || sending} className="w-full gap-2">
...
export default ShareAIChatDialog;
